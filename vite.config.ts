...
const WATCHED_CONFIG_FILES = ['package.json', 'config/policies.yml'];
...
const injectScriptFromQueryPlugin = () => {
  ...
  return {
    ...
    transformIndexHtml(html: string, ctx: IndexHtmlTransformContext): IndexHtmlTransformResult{
      ...
      let appName;
      try {
        const p = JSON.parse(readFileSync(join(root, 'package.json'), 'utf-8')) as { name?: string };
        appName = p.name;
      } catch {
...
export default defineConfig({
  ...
  base: './',
  define: {
    BUILD_NUMBER: JSON.stringify(pkg.buildNumber || 0),
